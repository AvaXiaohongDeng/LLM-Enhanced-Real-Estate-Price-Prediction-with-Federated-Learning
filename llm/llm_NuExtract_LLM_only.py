...
trunc_log_path = os.path.join(output_dir, "truncated_rows_log_llmonly.txt")  # Changed log filename
...
final_file = os.path.join(output_dir, "df_final_with_extracted_features_LLM_ONLY.csv")  # Changed output filename
json_file = os.path.join(output_dir, "raw_outputs_LLM_ONLY.jsonl")  # Changed JSON filename
...
# ---------- 2.3 JSON template and keyword lists (kept for reference but NOT used in merging) ----------
...
for start in range(resume_from, 20, chunk_size): # use n_rows to replace 500 after testing
    ...
    for raw_text in raw_outputs:  # Note: listing_text not needed since we're not merging
